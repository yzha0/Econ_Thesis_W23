# ---------------------------------------------------------------------------------------------
# File: Initial_Analysis.R
# By: Eric Zhao
# Date: 03/10/2024
# Description: This file perform initial summary and analyses of data
# ---------------------------------------------------------------------------------------------

#load-in data


nyCounty<-counties(state = "NY") %>%select(NAME, geometry)
...
ggplot(nyCounty_join_2021) +
  geom_sf(aes(fill = numeric.sd), alpha=0.8, col="white") +
  scale_fill_viridis_c(name = "sd recipiency rate") +
  ggtitle("sd of recipiency rate in NY counties")
